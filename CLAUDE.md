...
| Conforming refinement | [tasks/conforming-refinement.md](tasks/conforming-refinement.md) (design) + [tasks/conforming-refinement-debug.md](tasks/conforming-refinement-debug.md) (Task 8 sub-tasks D1–D8) | In progress — Tasks 1–7 done, **Task 8 (verification) nearly done**. D0–D6 done: **all 28 test registrations pass** at ranks 1–5 on both backends, so the suite has **zero open failures**. One recorded design limit (Decision 11): the *visible* layer is rank-count dependent up to blue closure diagonals — a geometric tie-break was implemented, measured and reverted because the closure cannot see the positions it needs (risk point 4); everything else is invariant and asserted. Remaining: D7 (calibrate `conforming_quality`'s bounds — `maxQ` is still rising at round 8) and D8 (close out: full gate, `ctest -L unit`, `format-check`, test totals, default-flip decision). **Read the debug file to resume.** |
