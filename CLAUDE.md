...
| Conforming refinement | [tasks/conforming-refinement.md](tasks/conforming-refinement.md) (design) + [tasks/conforming-refinement-debug.md](tasks/conforming-refinement-debug.md) (Task 8 sub-tasks D1–D8) | In progress — Tasks 1–7 done, **Task 8 (verification) nearly done**. D0–D7 done: **all 28 test registrations pass** at ranks 1–5 on both backends, so the suite has **zero open failures**. Shape quality is now measured rather than assumed (Decision 12): over 16 rounds the worst radius ratio saturates by round 11 and is flat through 16, so `conforming_quality` is recalibrated and **promoted into the gate** (tiers are now 140 `regression` / 62 `unit` instances). One recorded design limit (Decision 11): the *visible* layer is rank-count dependent up to blue closure diagonals — a geometric tie-break was implemented, measured and reverted because the closure cannot see the positions it needs (risk point 4); everything else is invariant and asserted. Remaining: D8 only (close out: full gate, `ctest -L unit`, `format-check`, test totals, default-flip decision). **Read the debug file to resume.** |
