...
| Conforming refinement | [tasks/conforming-refinement.md](tasks/conforming-refinement.md) (design) + [tasks/conforming-refinement-debug.md](tasks/conforming-refinement-debug.md) (Task 8 sub-tasks D1–D8) | In progress — Tasks 1–7 done, **Task 8 (verification) under way**. D0–D4 done: **all 28 test registrations now have a verdict and 25 pass** at ranks 1–5 on both backends, including `refine_splitedges`, `refine_conforming`, `conforming_quality` and `markquality_conforming`. Open: D5 (`conforming_migrate`) and D6 (`conforming_operators`, `conforming_determinism`) abort at np≥2 — pre-existing; then D7 (calibrate `conforming_quality`'s bounds — `maxQ` is still rising at round 8) and D8 (close out). **Read the debug file to resume.** |
