...
- `Mesh<Scalar, VertexFields, EdgeFields, FaceFields, MemorySpace, ExecSpace>` —
  **scalar precision is a template parameter** (never hard-code `double`).
- **Compile-time Cabana field pack** per entity kind for arbitrary user data
  (e.g. `VertexFields<Pos3, Vort2>`); every field haloes/migrates with its entity.
...
| 2 | Core data model: `Mesh<...>` template skeleton; core topology AoSoAs + templated user-state AoSoA (field pack, `position` mandatory `Scalar[3]`); 128-bit structured-key utils (edge key, midpoint/child derivation); dense-local-index + canonical-key side table; CSR vertex-adjacency container. Convert INTERFACE target to real lib if needed. | Start: empty `src/`. End: headers under `src/mesh/`; mesh constructs empty; keys + CSR usable. | Unit (SERIAL+HIP), for **both `double` and `float`**: structured-key determinism (ordering-invariant, same key regardless of endpoint order); AoSoA build; field-pack slice read/write. | Sonnet (keys Opus-spec'd) | Final gid/key struct + footprint decision (128-bit in AoSoA vs side table); field-pack declaration syntax; any Cabana template friction on HIP. |
...
| 6a | Local 1→4 red refinement (single rank): midpoint vertices via structured keys; 4 child faces; connectivity rebuild. | Start: 3. End: `refineLocal(face_mask)`. | Unit: Euler holds post-refine; child/parent incidence correct; midpoint shared between sibling faces. | Sonnet | Child-face vertex/edge ordering convention; how `level` is propagated. |
